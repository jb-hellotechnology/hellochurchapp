...
class PerchDB_MySQL
{
   ...
   public function implode_for_sql_or($rows, $force_int=false)
   {
   echo "implode_for_sql_in";
           print_r($rows);
        $sqlrows=array();
        foreach($rows as $key => &$item) {
          	if ($force_int) $item = (int)$item;
              $item = $this->pdb($item);
              array_push( $sqlrows,$key."=".$item);
         }

         return implode(' or ', $sqlrows);
    }
	...
	public function implode_for_sql_like_or($col,$rows, $force_int=false)
    {
   ...
   echo "implode_for_sql_in";
        ...
        foreach($rows as $key => &$item) {
          	if ($force_int) $item = (int)$item;
              $item = $this->pdb('%'.$item.'%');
              array_push( $sqlrows,$col." LIKE ".$item);
         }
