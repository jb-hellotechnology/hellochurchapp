...
class PerchFactory
{
    ...
    public function get_by($col, $val, $order_by_col=false, $Paging=false)
    {
        ...
        //print_r($val);
       // echo "*******";
        //print_r(in_array('op', array_keys($val)));
        if (is_array($val)) {
            if(in_array('op', array_keys($val))){
                // echo 'heree';
                //implode_for_sql_or
                if($val["op"]=="between"){
                 $sql    = $select . ' * FROM ' . $this->table . ' WHERE ' . $col . ' between ('.$this->db->pdb($val["fromdate"])  .') AND  ('.$this->db->pdb($val["todate"])  .') '.$this->standard_restrictions();

                }elseif($val["op"]=="likeor"){

                 $sql    = $select . ' * FROM ' . $this->table . ' WHERE  ('.$this->db->implode_for_sql_like_or($col,$val) .') '.$this->standard_restrictions();


                }else{


                      $sql    = $select . ' * FROM ' . $this->table . ' WHERE  ('.$this->db->implode_for_sql_or($val) .') '.$this->standard_restrictions();
                }
