		...
		public static function output_debug($return_value = false, $time = false)
		{
			...
			if ($Perch->debug == true) {
				...
				if (PerchUtil::count($messages)) {
					...
					$out .= ' 	<th>Debug Message - Perch '.$Perch->version.' '.(strpos(PERCH_LICENSE_KEY, '-LOCAL-')>2?'LTM':'').'</th>';
					$out .= ' </tr>';
		...
		public static function subnav($CurrentUser, $pages, $Lang = false)
		{
			...
			if (PerchUtil::count($pages)) {
				...
				foreach ($pages as $page) {
                    ...
                    $feature= $MenuItemFeatutes-> get_by_title($page['label']);

					// Runway?
					//if (isset($page['runway']) && $page['runway'] == true && !PERCH_RUNWAY) {
					if (PerchUtil::count($feature)&& !$feature['featureActive']){
						continue;
					}
